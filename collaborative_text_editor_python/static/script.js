...
server_socket.on('connect', function() {
	server_socket.emit('request_doc_list', {
		"userID": userId
	});
});
...
server_socket.on('response_doc_list', function(doc_list) {
	...
	if (parseInt(choice) == doc_list.length) {
		...
		server_socket.emit("create_new_doc", {
			"userID": userId,
		...
		server_socket.emit("request_doc_content", {
			"docName": doc_name,
			"docID": doc_ID,
			"userID": userId
		});
